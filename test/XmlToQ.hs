...
xmlToQ :: NameGenerator -> String -> FilePath -> ExpQ
xmlToQ ng modPrefix file =
    let p = (modPrefix ++)
        app a b = appE (varE (mkName a)) b
        elemToQ (Elem en atts content) =
          -- attributes
          let elemWithAtts = foldl (\a (an, attV) -> appEn ( varE $ mkName $ p $ (uiAddAttr ng) an )
                                                      [a, ( stringE $ show attV )] ) (varE $ mkName $ p $ (uiElName ng) $ en)  atts
          -- content
