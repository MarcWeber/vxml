...
elementState n H.EMPTY = do 
      addInfo n "empty"
      return $ (conT ''T.WeakValidation, [])
elementState n H.ANY = do
      addInfo n "any"
      ng <- gets nameGen
      return $ (conT ''T.WeakValidation, [childOk ng n Nothing])
elementState n (H.Mixed H.PCDATA) = do
      addInfo n "pcdata"
      ng <- gets nameGen
      return $ (conT ''T.WeakValidation, [childOk ng n (Just PCDATA)])
elementState n (H.Mixed (H.PCDATAplus list)) = do
      let l = nub list
      addInfo n $ "pcdata " ++ (show l)
      ng <- gets nameGen
      return $ (conT ''T.WeakValidation
                , childOk ng n (Just PCDATA) : map (childOk ng n . Just . T) l)
elementState n (H.ContentSpec spec) = do
      let l = nub $ contentSpecToList spec
      addInfo n (show l)
      ng <- gets nameGen
      return $ (conT ''T.WeakValidation
                , map (childOk ng n . Just . T) l)
